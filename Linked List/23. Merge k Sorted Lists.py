...
class Solution:
    def mergeKLists_divide_conquer(self, lists: List[ListNode]) -> ListNode:
        # divide and conquer
    ...
    def mergeKLists(self, lists: List[ListNode]) -> ListNode:
        ...
        if not lists or all([not node for node in lists]):
            return None
        dummy = ListNode(0)
        cur = dummy
        heap = [(node.val, index, node)
                for index, node in enumerate(lists) if node]
        heapq.heapify(heap)
        while heap:
